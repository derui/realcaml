...
module Region = struct
  ...
  let recent_point ~point t =
    ...
    let edge2 = V.inverse edge1 and base1 = point -: e1 and base2 = point -: e2 in
    let contained =
      List.for_all ~f:ident [ is_contain edge1 base1; is_contain edge2 base2; is_contain enormal point ]
    in
...
let make (v1, v2) normal =
  let open V.Open in
  let edge_normal = V.cross ~left:(v2 -: v1) ~right:normal |> V.normalize in
