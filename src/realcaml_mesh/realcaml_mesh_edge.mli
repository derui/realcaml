...
type t = {
  ...
  (* vertex ids that are included this edge.  *)
}

val alloc_edge_id: vertex_ids array -> t array
(* [alloc_edge_id edges] get to be allocated id for each edge.
   Notice, an edge that is gotten this function is only as `convex' edge.
*)
