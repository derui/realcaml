open Core.Std
module Opt = Realcaml_engine_option
module State = Realcaml_rigid_body.State
module Sweep_prune = Realcaml_engine_sweep_prune
module Separating_axis = Realcaml_engine_separating_axis
...
type pair_update_type = Remove | Update of (U.vec * float) | New of (U.vec * float)
...
let make ?(option=Opt.empty()) () = {
  ...
  pair_swap = 0;
  pair_count = [|0; 0|];
  pair = Array.make_matrix 2 option.Opt.max_pairs Pair.empty;
}

let add_body engine body =
  {engine with sweep_prune = Sweep_prune.add engine.sweep_prune body}
...
let sort_pair pair =
  let cmp a b = Int64.compare a.Pair.key b.Pair.key in
  Array.stable_sort ~cmp pair;
  pair
...
let update_contact_points bodies closest pair =
  ...
  let body_a = bodies.(to_int pair.Pair.indexA)
  and body_b = bodies.(to_int pair.Pair.indexB) in
...
let narrow_phase engine =
  ...
  and bodies = engine.sweep_prune.Sweep_prune.bodies in
  let is_separate body_a body_b =
    Separating_axis.judge_intersect ~body_a ~body_b in
...
let solve_constraints engine =
  ...
  let setup_solver = function
    | Some(o) -> Some(Solver.setup_solver_body o)
    | None -> None in
  ...
  {engine with sweep_prune = {engine.sweep_prune with Sweep_prune.bodies = bodies}}
...
let update_bodies engine =
  ...
  let time_step = engine.engine_option.Opt.time_step
  and bodies = engine.sweep_prune.Sweep_prune.bodies  in
  ...
  let calc_delta_orientation angular time_step =
    ...
    Q.make ~angle:(V.norm angular) ~axis () in
