...
let setup_constraint (bodyA, solverA) (bodyB, solverB) contact pair_type opt =
  ...
  let setup_contact_point cp =
    let rA = Q.rotate (State.orientation (RigidBodyInfo.state bodyA)) (ContactPoint.pointA cp)
    and rB = Q.rotate (State.orientation (RigidBodyInfo.state bodyB)) (ContactPoint.pointB cp) in
    ...
    let setup axis = let denom = V.dot (M3.mult_vec k axis) axis in
                     ...
                     let rhs = rhs -.
                       (opt.contact_bias *. (min 0.0 |< ContactPoint.distance cp)) /. opt.time_step in
                     let rhs = rhs *. 1.0 /. denom in
                     Constraint.make ~axis ~inv:(1.0 /. denom) ~rhs ~lower:0.0 ~upper:max_float ~accum:0.0 in
    let tangent1 = V.normalize (V.add (V.normal_axis `X) (ContactPoint.normal cp)) in
    let tangent2 = V.normalize (V.add tangent1 (ContactPoint.normal cp)) in
    let constraint1 = setup (ContactPoint.normal cp)
    and constraint2 = setup tangent1
    and constraint3 = setup tangent2 in
    let constraints = ContactPoint.constraints cp in
    constraints.(0) <- constraint1;
    constraints.(1) <- constraint2;
    constraints.(2) <- constraint3;
    cp in
  let setuped = Array.map setup_contact_point (Contact.contact_points contact) in
  Contact.set_contact_points contact setuped
;;
...
let solve (bodyA, solverA) (bodyB, solverB) contact opt =
  ...
  let solve_per_contact_point solverA solverB cp =
    ...
    let ctraint = (ContactPoint.constraints cp).(0) in
...
let update_constraint contact =
  let cps = Contact.contact_points contact in
  let friction = Contact.friction contact in
  ...
  let update_constraint_for_cp cp =
    let ctraint = (ContactPoint.constraints cp).(0) in
    let max_friction = friction *. (Constraint.accum_impulse ctraint |> abs_float) in
    let ctraint1 = (ContactPoint.constraints cp).(1)
    and ctraint2 = (ContactPoint.constraints cp).(2) in
    (ContactPoint.constraints cp).(1) <- new_constraint ctraint1 max_friction;
    (ContactPoint.constraints cp).(2) <- new_constraint ctraint2 max_friction;
    cp in
  let updated_cps = Array.map update_constraint_for_cp cps in
  Contact.set_contact_points contact updated_cps
;;
