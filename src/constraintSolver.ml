...
module Solver = struct
  ...
  let calc_delta impulse ct va vb =
    ...
    let impulse = max impulse low_lim |> min up_lim in
  ...
  let update_solver (impulse, delta) solver ct r operate =
    ...
    let calc_vec f =
      {solver with
        accum_impulse = delta;
        SolverBody.delta_linear_velocity = f solver.SolverBody.delta_linear_velocity
  ...
  let solve_per_contact_point solverA solverB cp =
    ...
    safe_hd cp.ContactPoint.constraints >>=
      (fun ctraint ->
        ...
        Printf.printf "impulse delta : %f : accum %f\n" delta_impulse accum_impulse_delta;
