...
(* Result of functions in this module. That types are each positions to first and second mesh, and last float is
   distination between them. *)
type t = {
  normal : U.Types.vec;
  point_a : U.Types.vec;
  point_b : U.Types.vec;
...
let sort_points list = List.sort ~compare:(fun { depth = d1; _ } { depth = d2; _ } -> Stdlib.compare d1 d2) list

(* Check the plane to be equal direction for normal. *)
let is_observe_face plane normal =
  let open V.Open in
  let open Float in
  plane.Mesh.Facet.normal *: normal >= 0.0
...
(* TODO: 最近接点を探す際、どちらか一方のどれかの形状を座標系として 利用できるようにする。bodyが入れ替わっても、座標系は変わってはならない。 最終的に取得した点はワールド座標系における衝突点として、それぞれの
   ローカル座標における衝突点を最終的に必要とする。ここでは、逆行列を必ず求める必要が あるので、簡便のために、正当ではない逆行列を返す可能性があるような関数を4x4行列に 作っておく。 *)
let get_plane_closest_points (axis, _) body_a body_b trans_mat =
  ...
  let world_b =
    ...
    let inverted = match M.inverse world_a with Some m -> m | None -> M.identity S.four in
    Rigid_body_info.get_world_transform body_b *: inverted *: trans_mat
  in

  (* TRANSLATE: それぞれのメッシュについて、offsetを反映させた上で実行させる。 *)
  let contacts =
    Array.map shapes ~f:(fun shape ->
              ...
              let points =
                Array.map shapes ~f:(fun shape ->
                    ...
                    Array.map mesh_b ~f:(fun vert ->
                        let point = Voronoi.recent_of_region ~point:vert voronoi_region in

                        { normal = axis; point_a = point; point_b = vert; depth = V.sub point vert |> V.norm })
                    |> Array.to_list)
                |> Array.to_list
              in
              match List.concat points |> sort_points with [] -> [] | hd :: _ -> [ hd ]
            else [])
        |> Array.to_list |> List.concat)
    |> Array.to_list
  in
  match contacts with [] -> [] | _ -> List.concat contacts
...
let get_edge_closest_points (axis, _) body_a body_b trans_mat =
  ...
  let world_b =
    ...
    let world_a_inv = match M.inverse world_a with Some m -> m | None -> failwith "No have inverse matrix" in
    let open M.Open in
    world_b *: world_a_inv *: trans_mat
  in
  ...
  let contacts =
    Array.map shapes ~f:(fun shape ->
        ...
        let points =
          Array.map mesh.Mesh.Mesh.edges ~f:(fun edge_a ->
              (* TRANSLATE: Body Bの各shapeにおけるそれぞれの頂点について処理をする *)
              let points =
                Array.map body_b.RBI.collidable.RI.Collidable.shapes ~f:(fun shape_b ->
                    let mesh_b = shape_b.RI.Shape.mesh in
                    let vertices = convert_vertices world_b mesh_b in
                    let points =
                      Array.map mesh_b.Mesh.Mesh.edges ~f:(fun edge_b ->
                          let edge_a = edge_to_segment shape.RI.Shape.mesh.Mesh.Mesh.vertices edge_a
                          and edge_b = edge_to_segment vertices edge_b in
                          let module L = Typedvec.Algebra.Line in
                          let e1, e2 =
                            match L.nearest_point ~a:edge_a ~b:edge_b () with
                            | L.Collide s -> s
                            | L.Nearest s -> s
                            | L.Parallel  -> failwith "edges are parallel"
                          in
                          { normal = axis; point_a = e1; point_b = e2; depth = V.sub e1 e2 |> V.norm })
                      |> Array.to_list
                    in

                    match sort_points points with [] -> [] | hd :: _ -> [ hd ])
                |> Array.to_list
              in
              match List.concat points |> sort_points with [] -> [] | hd :: _ -> [ hd ])
          |> Array.to_list
...
let get_inverse_translation axis dist =
  ...
  let af = A.make Typedvec.Size.three in
...
let get_closest_point ~axis ~dist body_a body_b =
  ...
  let points = List.concat [ edge_base_closests; a_plane_base_closests; b_plane_base_closests ] |> sort_points in
  match points with
  | []      -> failwith "Not found closest point"
  | hd :: _ ->
      let open A.Open in
      let point_b = U.Vec.to_four hd.point_b *> reverse_mat |> U.Vec.to_three in
      { hd with point_b; depth = dist }
